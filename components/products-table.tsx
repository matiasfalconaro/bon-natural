...
import { useAdminProducts, AdminProductRow } from "@/hooks/use-admin-products"

export function ProductsTable() {
  const { products: data, loading } = useAdminProducts()

  const [sorting, setSorting] = useState<SortingState>([])
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({})
  const [rowSelection, setRowSelection] = useState({})
  const [editItem, setEditItem] = useState<AdminProductRow | null>(null)
  const [deleteItem, setDeleteItem] = useState<AdminProductRow | null>(null)
